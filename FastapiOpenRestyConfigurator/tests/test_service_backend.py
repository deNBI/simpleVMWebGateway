...
file_path_example_1 = (
    "/var/forc/backend_path/1234567890%testuser%animal_100%testtemplate%v01%0.conf"
)
file_path_example_2 = (
    "/var/forc/backend_path/9876543210%otheruser%cat_200%othertemplate%v02%1.conf"
)


@pytest.mark.asyncio
async def test_get_backend_by_id(backend_out_factory):
    backendout1 = backend_out_factory(id=123)
    backendout2 = backend_out_factory(id=456)


    with patch(
        "app.main.service.backend.get_backends",
        return_value=[backendout1, backendout2],
...
@pytest.mark.asyncio
async def test_get_backend_by_id_not_found(backend_out_factory):
    backendout1 = backend_out_factory(id=123)
    backendout2 = backend_out_factory(id=456)
...
@pytest.mark.asyncio
async def test_get_filepath_by_id(backend_out_factory):
    backendout1 = backend_out_factory(id=1234567890, file_path=file_path_example_1)
    backendout2 = backend_out_factory(id=9876543210, file_path=file_path_example_2)
...
@pytest.mark.asyncio
async def test_get_filepath_by_id_not_found(backend_out_factory):
    backendout1 = backend_out_factory(id=1234567890)

    with pytest.raises(NotFound):
        with patch(
            "app.main.service.backend.get_backends",
            return_value=[backendout1],
        ) as mock_get_backends:
